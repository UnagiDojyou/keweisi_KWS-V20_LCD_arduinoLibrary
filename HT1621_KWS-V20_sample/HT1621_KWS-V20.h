...
public:
    ...
    */
    ...
    void writeV20(uint8_t address, uint8_t data);
    /*
     * writeのKWS-V20版
     * addressは表示させたい7セグの指定、dataは表示させるセグメントの指定。abcdefghで指定する。
    */

    void writeCharV20(uint8_t address, uint8_t c, bool decimal);
    /*
     * writeCharのKWS-V20版
     * addressで表示させたい7セグの指定、cは表示させたい数字、decimalは1で小数点の表示、0で非表示
    */

    void writeV20voltage(float N,int F,bool V);
    /*
     * 電圧表示部分に表示をさせる。Nは電圧、Fは少数F桁まで表示、Vは1で「V」を表示・0で非表示
     * なお、F=0のときは999~0、F=1のときは99.9~0、F=2のときは9.99~0までしか表示できない。範囲外はどうなるか不明
    */

    void writeV20current(float N,int F,bool A);
    /*
     * 電流表示部分に表示をさせる。Nは電流、Fは少数F桁まで表示、Aは1で「A」を表示・0で非表示
     * writeV20voltageの電流版。ただし、小数点は第2位しか指定できない。F=1としても、F=2となる。
     * F=0のときは999~0、F=1,2のときは9.99~0となっている。範囲外はどうなるか不明
    */

    void writeV20time(int h,int m,bool colon,bool T);
    /*
     * 時間表示部分に表示をさせる。hは時、mは分、colonは「:」の表示・非表示、Tは「T」の表示・非表示
    */

    void writeV20mAh(long N,bool mAh);
    /*
     * mAh部分に表示させる。Nで表示させる数字、mAhは「mAh」の表示・非表示
    */
};
